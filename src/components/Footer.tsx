...
import { categoryTe, hoursTe } from "../lib/i18n-dict";
  ...
  return (
    <footer className="bg-charcoal text-white/80">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              <p className="font-display text-[15px] font-extrabold text-white">{t("brandName")}</p>
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/50">{t("brandTagline")}</p>
            </div>
          </div>
          ...
          >
            <MessageCircle size={15} /> {t("talkWhatsApp")}
          </a>
        </div>

        <div>
          <h4 className="font-display text-sm font-bold uppercase tracking-wider text-warm-yellow">{t("navProducts")}</h4>
          <ul className="mt-4 space-y-2.5 text-sm">
            {categories.map((c) => (
              <li key={c.slug}>
                <Link to={`/products?category=${c.slug}`} className="transition hover:text-warm-yellow">
                  {pick(c.name, categoryTe[c.slug] ?? c.name)}
                </Link>
              </li>
            ))}
          </ul>
        ...
        <div>
          <h4 className="font-display text-sm font-bold uppercase tracking-wider text-warm-yellow">{t("navByCrop")}</h4>
          <ul className="mt-4 space-y-2.5 text-sm">
            {crops.map((c) => (
              <li key={c.slug}>
        ...
        <div>
          <h4 className="font-display text-sm font-bold uppercase tracking-wider text-warm-yellow">{t("footerVisitHeading")}</h4>
          <ul className="mt-4 space-y-3.5 text-sm">
            ...
            {STORE.hours.map((h) => (
              <li key={h.day} className="flex gap-2.5">
                ...
                <span>{pick(h.day, hoursTe[h.day] ?? h.day)}: {pick(h.time, hoursTe[h.time] ?? h.time)}</span>
              </li>
            ))}
          </ul>
          <a
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-5 text-xs text-white/45 sm:flex-row">
          <p>{pick(`© ${new Date().getFullYear()} ${STORE.name}, Dharmavaram. All rights reserved.`, `© ${new Date().getFullYear()} ${STORE.name}, ధర్మవరం. అన్ని హక్కులు రిజర్వ్ చేయబడినవి.`)}</p>
